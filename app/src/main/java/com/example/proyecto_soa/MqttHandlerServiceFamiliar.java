    ...
    private final BroadcastReceiver publishStepReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            if ("PASOS_ACTUALIZADOS".equals(intent.getAction())) {
                String message = intent.getStringExtra("pasos");

                if (message != null) {
                    Log.d("MQTT", "recibe broadcast");
                    Log.d("MQTT", message);

                    publish(configMQTT.TOPIC_STEP_EMQX, message);
                    Log.d("MQTT", "publica broadcast");
                }
            }
        }
    ...
    */
    ...
    public void messageArrived(String topic, MqttMessage message) throws Exception {
        Thread thread = new Thread(new Runnable() {
            @Override
            public void run() {
               if (topic.equals(configMQTT.TOPIC_ALARM_EMQX) && "FAMILIAR".equals(rol)) {
                    ...
                    sendBroadcast(i);
                } else if (topic.equals(configMQTT.TOPIC_ON_OFF_EMQX) && "FAMILIAR".equals(rol)) {
                    String alarm_state = message.toString();
                    ...
                    sendBroadcast(i);
                } else if (topic.equals(configMQTT.TOPIC_STEP_EMQX) && "FAMILIAR".equals(rol)) {
                    ...
                    sendBroadcast(i);
                } else if (topic.equals(configMQTT.TOPIC_LOCATION_EMQX) && "FAMILIAR".equals(rol)) {
